...
const ParentDashboard = ({ registrationId }) => {
  const path = "https://localhost:7051/api/parents/" + registrationId;
  ...
  const handleChange = (e) => {
    ...
    if (name === "country") {
      ...
      setParentData((prevData) => ({
        ...prevData,
        country: value,
        state: "", // Clear the state when country changes
      }));
    } else {
      setParentData((prevData) => ({
        ...prevData,
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      try {
        await axios.put(
          `https://localhost:7051/api/Parents/${registrationId}`,
          parentData
        );
  ...
  return (
    <div className="form-container">
      ...
      <form onSubmit={handleSubmit} className="registration-form">
        ...
        <div className="form-row">
          <label className="address-title" htmlFor="address">Address</label>
          <input
            type="text"
            id="address"
            name="address"
